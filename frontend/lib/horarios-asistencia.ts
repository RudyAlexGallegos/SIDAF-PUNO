...
// Días obligatorios de asistencia (según el plan)
// Lunes=1, Martes=2, Jueves=4, Viernes=5, Sábado=6
...
export function esDiaObligatorio(fecha: Date | string): boolean {
  ...
  return DIAS_OBLIGATORIOS.includes(d as number)
}

export function getTipoDia(fecha: Date | string): TipoDia {
  const d = getDiaSemana(fecha)
  if (DIAS_OBLIGATORIOS.includes(d as number)) return "OBLIGATORIO"
