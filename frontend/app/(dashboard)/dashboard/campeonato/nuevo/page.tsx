...
export default function NuevoCampeonatoPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const nuevoCampeonato: CampeonatoAPI = {
            ...
            fechaInicio: formData.fechaInicio ? new Date(formData.fechaInicio).toISOString().split('T')[0] : undefined,
            fechaFin: formData.fechaFin ? new Date(formData.fechaFin).toISOString().split('T')[0] : undefined,
            estado: formData.estado || "programado",
            direccion: formData.direccion || "",
            ciudad: formData.ciudad || "",
            diasJuego: JSON.stringify(formData.diasJuego || []),
            horaInicio: formData.horaInicio || "",
            horaFin: formData.horaFin || "",
            equipoIds: JSON.stringify(formData.equipoIds || []),
        }

        try {
            await createCampeonato(nuevoCampeonato)
            router.push("/dashboard/campeonato")
        } catch (error) {
            console.error("Error al crear campeonato:", error)
            alert("Error al crear el campeonato. Por favor, intente nuevamente.")
        }
    }
