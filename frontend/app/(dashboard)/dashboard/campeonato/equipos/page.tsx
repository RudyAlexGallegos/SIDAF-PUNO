...
export default function EquiposPage() {
    ...
    const filtered = useMemo(() => {
        ...
        if (provinciaFilter !== "todos") {
            result = result.filter((e: EquipoAPI) => e.provincia === provinciaFilter)
        }

        if (divisionFilter !== "todos") {
            result = result.filter((e: EquipoAPI) => e.categoria === divisionFilter)
        }

        if (!q) return result
        return result.filter((e: EquipoAPI) =>
    ...
    if (loading) {
        return (
            <div className="container mx-auto py-8 px-4 max-w-7xl">
                <div className="text-center py-12">Cargando equipos...</div>
            </div>
        )
    }
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {filtered.map((equipo: EquipoAPI) => (
                        <Card key={equipo.id} className="group hover:shadow-xl transition-all duration-300 border-0 shadow-lg bg-white/80 backdrop-blur-sm overflow-hidden">
                            ...
                            <CardContent>
                                <div className="flex items-center gap-2 mb-4">
                                    {getDivisionBadge(equipo.categoria)}
                                </div>
