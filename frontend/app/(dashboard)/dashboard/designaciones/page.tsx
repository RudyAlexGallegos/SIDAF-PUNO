...
export default function DesignacionesPage() {
  ...
  const getStatusBadge = (fecha: string) => {
    ...
    if (diasDiferencia < 0) {
      return <span className="px-2 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-600">Finalizado</span>
    } else if (diasDiferencia === 0) {
      return <span className="px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-600 animate-pulse">Hoy</span>
    } else if (diasDiferencia <= 3) {
      <span className="px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-600">Próximo</span>
    } else {
      return <span className="px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-600">Programado</span>
    }
  }
